...
export function RecordTreatmentCard({
  ...
  const recordHref = selectedStep?.medicalRecordId
    ...
    : null;
  const pdfHref = `/records?planId=${treatment.id}&export=pdf`;
  const appointmentHref = appointment
    ? `/appointment?planId=${encodeURIComponent(treatment.id)}&stepId=${encodeURIComponent(selectedStep?.id ?? "")}`
    : "/appointment";
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#0058bc]">
                Quy trình {index + 1}
              </span>
          ...
          {visibleSteps.map((step, stepIndex) => {
            ...
            return (
              <button
                ...
                className={`flex min-w-[220px] items-start gap-3 rounded-2xl border px-3 py-3 text-left transition ${
                  selected
