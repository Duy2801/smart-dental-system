...
export function PaymentCheckoutView({
  ...
  useEffect(() => {
    const consultationId = bookingResult.consultation.id;
    if (!consultationId) return;

    const interval = setInterval(async () => {
      try {
        const info = await getConsultationPaymentInfo(consultationId);
        if (info.isPaid) {
          setIsPaid(true);
    ...
    <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-md border border-slate-100 space-y-6">
      <div className="flex items-center gap-3 text-emerald-600">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800">
            {isPaid ? "Đã Xác Nhận Thanh Toán Thành Công!" : "Đã Khởi Tạo Đơn Tư Vấn Thành Công!"}
          </h2>
      ...
      {isPaid ? (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-emerald-800 text-sm flex items-center gap-3">
          ...
          <div>
            <strong>Thanh toán đã được ghi nhận thành công!</strong>
            <p className="mt-0.5 text-xs text-emerald-700">
              Lịch tư vấn trực tuyến đã chính thức giữ vị trí. Bạn sẽ nhận được thông báo nhắc lịch 10 phút trước buổi hẹn.
            </p>
          </div>
        </div>
      ) : (
