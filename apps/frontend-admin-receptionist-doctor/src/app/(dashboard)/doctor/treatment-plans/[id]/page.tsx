  ...
  const handleStepStatus = async (step: PlanStep, newStatus: StepStatus) => {
    ...
    try {
      ...
      setPlan((prev) => {
        ...
        const completed = newSteps.filter(
    ...
    <div className="bg-slate-50/50 px-6 py-8">
      <div className="mx-auto max-w-4xl">
            ...
            <div className="rounded-xl bg-slate-50 px-4 py-3">
              ...
              <div className="mt-1 relative">
                <select
                  value={plan.status}
                  onChange={(e) =>
                    handlePlanStatusChange(e.target.value as PlanStatus)
                  }
                ...
                {planStatusChanging ? (
                  <SpinnerGap
                    size={12}
                    className="pointer-events-none absolute right-1.5 top-1/2 -translate-y-1/2 animate-spin"
                  />
                ) : (
                  <CaretDown
                    size={11}
        ...
        {plan.status !== "CANCELLED" && (
          <TreatmentPlanExplanation key={plan.id} planId={plan.id} />
        )}
                ...
                return (
                  <div key={step.id} className="group">
                    ...
                    <div
                      ...
                      onClick={() =>
                        setExpandedStep((p) => (p === step.id ? null : step.id))
                      }
                    >
                      {/* Step icon */}
