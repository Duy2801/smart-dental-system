  ...
  private async getRecentActivities() {
    ...
    return [
      ...
      ...patients.map((patient) => ({
        ...
        type: 'patient',
        title: 'Benh nhan moi',
        description: `${patient.fullName ?? patient.user?.fullName ?? 'Benh nhan'} da dang ky ho so`,
  ...
  private async getActionItems() {
    ...
    return [
      ...pendingAppointments.map((appointment) => ({
        ...
        desc: `${appointment.patient?.fullName ?? appointment.patient?.user?.fullName ?? appointment.creator.fullName} dat lich (${appointment.status}).`,
        time: appointment.createdAt.toISOString(),
        action: 'Xem lich hen',
