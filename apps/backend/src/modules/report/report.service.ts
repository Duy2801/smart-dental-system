...
export class ReportService {
    ...
    return {
      statCards: [
        ...
        },
        {
          ...
          trendLabel: 'điểm % so với kỳ trước',
        },
        {
          label: 'Đánh Giá (TB)',
  ...
  private async getRecentActivities() {
    ...
    return [
      ...appointments.map((appointment) => ({
        ...
        type: 'appointment',
        title: 'Đặt lịch mới',
        description: `${appointment.patient?.fullName ?? appointment.patient?.user?.fullName ?? appointment.creator.fullName} - ${appointment.treatmentMethod?.name ?? 'Dịch vụ'}`,
      ...
      ...patients.map((patient) => ({
        ...
        type: 'patient',
        title: 'Bệnh nhân mới',
        description: `${patient.fullName ?? patient.user?.fullName ?? 'Bệnh nhân'} đã đăng ký hồ sơ`,
      ...
      ...payments.map((payment) => ({
        ...
        type: 'payment',
        title: 'Thanh toán',
        description: `${payment.invoice.invoiceCode} - ${Number(payment.amount).toLocaleString('vi-VN')}đ`,
  ...
  private async getActionItems() {
    ...
    return [
      ...pendingAppointments.map((appointment) => ({
        ...
        desc: `${appointment.patient?.fullName ?? appointment.patient?.user?.fullName ?? appointment.creator.fullName} đặt lịch (${appointment.status}).`,
        time: appointment.createdAt.toISOString(),
        action: 'Xem lịch hẹn',
      ...
      ...unpaidInvoices.map((invoice) => ({
        ...
        desc: `${invoice.invoiceCode} trị giá ${Number(invoice.finalAmount).toLocaleString('vi-VN')}đ.`,
        time: (invoice.issuedAt ?? new Date()).toISOString(),
        action: 'Xem tài chính',
        href: '/admin/finance',
      })),
    ].slice(0, 5);
  }
  ...
  private async getTodaySchedule(range: DateRange) {
    ...
    return appointments.map((appointment) => ({
      ...
      patient_name: appointment.patient?.fullName ?? appointment.patient?.user?.fullName ?? appointment.creator.fullName,
